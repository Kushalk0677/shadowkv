...
def test_config_update_invalidates_cached_get() -> None:
    config = RuntimeConfig(Path(__file__).with_name('missing_runtime_config_for_update_test.yaml'))

    assert config.get('policy.utility.semantic_threshold', 'missing') == 'missing'
    config.update({'policy.utility.semantic_threshold': 0.77})

    assert config.get('policy.utility.semantic_threshold', 'missing') == 0.77
