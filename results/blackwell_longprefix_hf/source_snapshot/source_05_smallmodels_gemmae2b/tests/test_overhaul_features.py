...
def test_semantic_prompt_mode_rotates_paraphrased_scaffolds_and_sets_equivalence_key(monkeypatch):
    monkeypatch.setenv('SHADOWKV_SEMANTIC_LONG_PREFIX_MODE', 'variant_scaffold')
    base_prompt = 'Instruction: do the thing\nAssistant response:'
...
def test_semantic_prompt_mode_common_scaffold_keeps_exact_shared_prefix(monkeypatch):
    monkeypatch.setenv('SHADOWKV_SEMANTIC_LONG_PREFIX_MODE', 'common_scaffold')
    monkeypatch.setenv('SHADOWKV_SEMANTIC_LONG_PREFIX_REPEATS', '4')
    base_prompt = 'Instruction: do the thing\nAssistant response:'
    p0, s0, m0 = _apply_prompt_mode('dolly', 'instruction', base_prompt, 'semantic', request_index=0)
    p1, s1, m1 = _apply_prompt_mode('dolly', 'instruction', base_prompt, 'semantic', request_index=1)
    assert p0.startswith(s0)
