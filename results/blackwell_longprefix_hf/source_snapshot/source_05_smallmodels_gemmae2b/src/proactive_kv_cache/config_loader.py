...
class RuntimeConfig:
    ...
    def __init__(self, path: str | Path | None = None) -> None:
        ...
        # Add caching to reduce file system access
        self._last_check_time: float = 0.0
        self._check_interval_seconds: float = 1.0  # Check for changes at most once per second
        self._cached_data: dict[str, Any] = {}
        self._cached_get_results: dict[str, Any] = {}
        self._last_get_cache_clear: float = 0.0
    ...
    def load(self, path: str | Path | None = None) -> None:
        with self._lock:
            ...
            if not self._path.exists():
                ...
                self._mtime_ns = None
                self._hash = None
                # Clear caches when config is reset
                self._cached_get_results.clear()
                self._last_get_cache_clear = time.time()
                return
    ...
    def reload_if_changed(self, force: bool = False) -> None:
        with self._lock:
            ...
            # Only check file system at most once per second to reduce overhead
            if not force and (current_time - self._last_check_time) < self._check_interval_seconds:
                return
            ...
            if force or self._mtime_ns != mtime_ns:
                self.load(self._path)
                # Clear cached get results when config changes
                self._cached_get_results.clear()
                self._last_get_cache_clear = current_time

    def snapshot(self) -> dict[str, Any]:
        current_time = time.time()
        # Use cached data if we've checked recently
    ...
    def get(self, dotted_path: str, default: Any = None) -> Any:
        ...
        cache_key = f"{dotted_path}||{default}"
        current_time = time.time()
        
        # Clear get cache periodically to prevent memory growth
        if current_time - self._last_get_cache_clear > 60.0:  # Clear cache every minute
            self._cached_get_results.clear()
            self._last_get_cache_clear = current_time
            
        if cache_key in self._cached_get_results:
            return self._cached_get_results[cache_key]
        ...
        for part in dotted_path.split('.'):
            if not isinstance(current, Mapping) or part not in current:
                result = default
                self._cached_get_results[cache_key] = result
                return result
            current = current[part]
        
        self._cached_get_results[cache_key] = current
        return current
