...
RUN_ID=gemma4_e2b_append_longprefix_n128_20260714
SMOKE_ID=gemma4_e2b_append_longprefix_smoke_20260714
RESULTS="$PROJECT/results_blackwell_semantic_seven_models_longprefix_n128_20260713"
SMOKE_RESULTS="$PROJECT/results_smoke_${SMOKE_ID}"
META="$RESULTS/metadata/e2b_append_20260714"
...
restore_production() {
  ...
  for _ in $(seq 1 120); do
    if curl -fsS http://127.0.0.1:8014/v1/models > "$META/production_models_after.json" 2>/dev/null; then
...
printf 'run_id=%s\nproject=%s\nresults=%s\nimage=%s\n' "$RUN_ID" "$PROJECT" "$RESULTS" "$IMAGE" > "$META/run_identity.txt"
printf '%s\n' "${MODELS[@]}" > "$META/requested_models.txt"
uname -a > "$META/uname.txt"
lscpu > "$META/lscpu.txt"
nvidia-smi -q > "$META/nvidia_smi_q_before.txt"
nvidia-smi --query-gpu=timestamp,name,driver_version,memory.total,memory.used,utilization.gpu,power.draw,temperature.gpu --format=csv > "$META/gpu_baseline_before.csv"
docker image inspect "$IMAGE" > "$META/docker_image_inspect.json"
docker run --rm --entrypoint /usr/bin/python3 "$IMAGE" -m pip freeze > "$META/runtime_versions.txt"
sha256sum \
...
cp "$PROJECT/OPERATOR_NOTES_20260710.md" "$META/"
cp "$RESULTS/_job_results.json" "$META/six_models_job_results.json"
cp "$RESULTS/all_results.csv" "$META/six_models_all_results.csv"
cp "$RESULTS/comparisons_vs_no_cache.csv" "$META/six_models_comparisons_vs_no_cache.csv"
cp "$RESULTS/reuse_path_breakdown.csv" "$META/six_models_reuse_path_breakdown.csv"
...
kill "$GUARD_PID" >/dev/null 2>&1 || true
wait "$GUARD_PID" 2>/dev/null || true
nvidia-smi --query-gpu=timestamp,name,driver_version,memory.total,memory.used,utilization.gpu,power.draw,temperature.gpu --format=csv > "$META/gpu_after.csv"
