...
**Paper claim:** ShadowKV++ achieves 1.365× mean speedup over no-cache with 0.156 waste, beating all baseline cache engines.
...
## 2. MeritKV Beats Cost-Only and ShadowKV
...
**Paper claim:** MeritKV handles mixed traffic with the highest speedup or lowest waste on 5 of 6 workloads, and waste control is where it separates from waste-unaware policies.
...
**Paper claim:** ShadowKV++ achieves +16.7% speedup over LMCache at 7B and +5.1% at 32B on SGLang RadixAttention.
...
**Paper claim:** ShadowKV++ achieves +19.0% speedup over no-cache at 32B and +24.5% at 7B on vLLM with APC, with matching GPU energy savings.
...
**Paper claim:** KV reuse fidelity varies by architecture. LLaMA and Gemma show strong fidelity (ROUGE-L > 0.96), while Qwen2 in float16 needs a quality guard (ROUGE-L 0.200).
...
**Paper claim:** Float16 precision amplifies KV cache drift. Qwen2's attention architecture shows approximately 10× more sensitivity than LLaMA/Gemma at float16.
...
**Paper claim:** ShadowKV++ detects semantic reuse opportunities that exact-prefix caches miss, recording non-zero semantic opportunity metrics across real datasets.
...
**Paper claim:** Three semantic ablation engines (scaffold-only, early-layer, logit-guarded) provide a speed-quality tradeoff curve and correctness-preserving boundaries for semantic KV reuse.
...
**Paper claim:** The coupled utility extension U(lambda) = U − λ · κ · B · max(e_w, 0.02) eliminates all Phi-3 failures (117 flips to 0) at λ = 0.15 with less than 0.7% mean speedup reduction.
