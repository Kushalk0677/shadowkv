                ...
                [
                    ...
                    ShadowKVEngine(
                        ...
                        policy=CostAwareSlackPolicy(
                            min_frequency=0.20 if backend.device.startswith('cuda') else 0.35,
                            token_benefit_ms=1.2 if backend.device.startswith('cuda') else 0.9,
                            speculation_penalty_ms=5.0 if backend.device.startswith('cuda') else 10.0,
                            min_prefix_len=24 if backend.device.startswith('cuda') else 16,
                            preferred_prefix_len=96 if backend.device.startswith('cuda') else 64,
                            max_prefix_len=192 if backend.device.startswith('cuda') else 128,
                            min_observations=3 if backend.device.startswith('cuda') else 5,
                            min_expected_net_ms=8.0 if backend.device.startswith('cuda') else 28.0,
                            benefit_cost_ratio=1.25 if backend.device.startswith('cuda') else 2.00,
